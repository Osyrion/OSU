...
class User extends Connection {
    ...
    public function getStudentExams() {
        $sql = "SELECT * FROM (SELECT courses.* FROM courses LEFT JOIN students2courses ON courses.id = students2courses.id_course WHERE students2courses.login_student = '".$_SESSION['user']."') as s2c LEFT JOIN exams ON exams.id_course = s2c.id WHERE exams.id NOT IN (SELECT id_exam FROM students2exams WHERE login_student = '".$_SESSION['user']."') AND exams.id IS NOT NULL;";
        return $this->getQuery($sql);
    }
    ...
    public function getAllStudentExams() {
        //$db = new connection();
        $sql = "SELECT students2exams.*, exams.*, courses.* FROM students2exams LEFT JOIN exams ON students2exams.id_exam = exams.id LEFT JOIN courses ON courses.id = exams.id_course WHERE students2exams.login_student = '".$_SESSION['user']."'";
    ...
    public function setCapacity(int $capacity, int $id) {
        $db = new connection();
        $sql = "UPDATE exams SET capacity = " . $capacity . " WHERE id = " . $id;
        $db->getQuery($sql);

        header("Refresh: 1");
    }

    public function unregisterExam(int $idExam, string $loginStudent) {
        $db = new connection();
        $sql = "DELETE FROM students2exams WHERE id_exam =". $idExam . " AND login_student ='" .$loginStudent. "'";
        $db->getQuery($sql);
